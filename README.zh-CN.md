...
- **ALLELE 行**：每个位点处的替代等位基因。每个唯一等位基因分配 ColorBrewer Set2 调色板中的一种颜色。文字颜色自动适配——深色背景用白字、浅色背景用深字——基于亮度感知对比度计算。
- **单倍型行**（H001、H002、...）：每行代表一种独特的单倍型模式。单倍型名称以 1.5 倍字号加粗显示。单元格显示该位点携带的等位基因，按等位基因调色板着色。空单元格表示参考等位基因。
- **群体列**（仅 `--population`）：提供群体分组文件时，每个群体（如野生种、地方品种、栽培种）对应一列，显示属于各单倍型的样本数。列背景以浅色调着色，视觉上将群体数据与变异数据区分。
- **n/N 列**：各单倍型频率，格式为计数/总数。
- **SnpEff 风格图例**（仅 `--gff`）：当 GFF 注释激活时显示在底部。仅展示功能类别颜色（CDS、UTR、exon、intron、intergenic），不展示等位基因颜色——保持图例紧凑，聚焦于功能解读。
...
- **底图**：从 GeoJSON 渲染的省界多边形（中国底图来自阿里云 DataV API）。浅色填充配细灰边框提供地理上下文，不与数据层争抢视觉注意力。
- **饼图**：在每个采样位置，饼图展示单倍型组成。每个扇形代表一种单倍型，按全局调色板着色。白色边线分隔扇形以保证清晰度。
- **√频率缩放**：符号半径与 √(样本总数) 成正比，与 R 的 `symbol.lim` 逻辑一致。这保证视觉面积——而非直径——与样本量线性成比例。`symbol_lim` 参数控制最小/最大半径范围。
